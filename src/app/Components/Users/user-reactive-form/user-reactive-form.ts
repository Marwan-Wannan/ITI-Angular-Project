...
export class UserReactiveForm {
  ...
  createMobileControl(): FormControl {
    return new FormControl('', [
      Validators.required,
      Validators.pattern(/^[+]?[\d\s\-\(\)]{10,15}$/) // Pattern for international mobile numbers
    ]);
  }
  ...
  onSubmit() {
    if (this.emailTaken('email')) {
      this.submitMessage = 'This email is already taken. Please use a different one.';
    ...
    if (this.userForm.valid) {
      ...
      this.userService.registerUser(userData).subscribe({
        ...
        error: (error) => {
          this.isSubmitting = false;
          this.submitMessage = 'Registration failed. ' + (error === 'email taken' ? 'Email is already taken.' : 'Please try again later.');
          this.submitMessageType = 'danger';
  ...
  emailTaken(controlName: string): boolean {
    this.userService.isEmailTaken(this.userForm.get(controlName)?.value).subscribe(isTaken => {
      console.log(isTaken);
